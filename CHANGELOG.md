...
## [1.38.0](https://github.com/FazeUrru/TODO-LOGO-AI/compare/v1.37.0...v1.38.0) · 12 sept 2026, 13:45 — *StreamDog ∞ infinito de verdad: Explorar ∞ en todas las categorías, reproductor turbo con lazyload y autoguardado total (seguir viendo + ajustes + gustos)*

> 💡 **En una frase:** el catálogo **ya no tiene fondo**: la sexta vista «Explorar» abre **todas las categorías** con **scroll infinito** (~2 000 fichas por categoría y subiendo), cada fila del inicio estrena su botón **«Ver todo»**, el **reproductor va turbo** (code-splitting con `lazy()`, preload progresivo metadata→auto, y tu velocidad y volumen de siempre autoguardados) y la casa por fin **aprende tus gustos**: la fila «Porque te gusta» recomienda por afinidad real — géneros y fuentes que consumes — sin que nada salga de tu dispositivo.

### Añadido
- **EXPLORAR ∞ (todas las categorías, sin fondo)** ♾️: 10 categorías (`CATEGORIAS_EXPLORAR` en cine.ts) con tres motores — **Archive.org** por colección (clásicos, film noir, ciencia ficción y terror, animación clásica, TV clásica, documentales; paginación nativa del advancedsearch), **Wikimedia Commons** por búsqueda con offset (cortos libres, animación libre, documentales libres) y **TVMaze** con TODO su catálogo de series por peso real. Hasta 50 páginas × 40 fichas por categoría; endpoint `?vista=explorar&cat=X&pagina=N` con caché caliente de 30 min por (cat, página), y el **cron empresarial** recalienta la primera página de las 10 cada hora. La grilla carga con **scroll infinito** (IntersectionObserver + centinela con rootMargin 600 px) y conserva el botón «Cargar más» como respaldo accesible.
- **Autoguardado de AJUSTES** ⚙️: `cine-gustos.ts` (puro y sin red) define `AjustesCine` — autoplay, carga infinita, velocidad por defecto y volumen por defecto — persistido con la autoreparación de la casa (`leerColeccion`/`guardarColeccion`, clave `streamdog.cine.v1.ajustes`). El panel «Ajustes» (nuevo botón en la cabecera del módulo) escribe al vuelo, y el **Reproductor obedece**: arranca con tu velocidad y tu volumen, y respeta el autoplay elegido (sin autoplay si lo prohibiste).
- **Autoguardado de GUSTOS + «Porque te gusta»** ✨: cada título que abres o añades a Mi lista alimenta `GustosCine` (contadores de géneros con techo y poda a 12, contadores de fuente, «últimos» 24 títulos; clave `streamdog.cine.v1.gustos`). La función pura `recomendadosPara()` rankea el pool ya cargado por **afinidad real** (género ×2 + fuente ×1, empate por valoración y orden alfabético) y **nunca recomienda lo que acabas de ver**. La fila «Porque te gusta» aparece en el inicio solo cuando hay aprendizaje honesto.
- **«Ver todo» en las filas del inicio**: las filas con categoría propia (Film noir, Ciencia ficción y terror, Dibujos animados clásicos, Televisión clásica, Documentales, Cine clásico libre, Explorar el archivo libre, Series del momento) saltan directo a su grilla infinita con un clic.

### Mejorado
- **Reproductor turbo** 🚀: **code-splitting** con `React.lazy` + `Suspense` (el modal del vídeo ya no pesa en el bundle inicial — se descarga en el momento del primer play, con spinner propio); **preload progresivo** (`metadata` al abrir, `auto` en cuanto hay play real — el búfer crece agresivo solo cuando vas a ver); y sincronización de ajustes en `loadedmetadata` y en cada cambio de volumen/velocidad (persistencia inmediata).
- **Lazyload visual en todo el catálogo**: las filas del inicio **no montan sus fichas** hasta que el IntersectionObserver las ve venir (rootMargin 400 px, una vez) — el arranque con ~20 filas ♾️ pasa de cientos de imágenes a solo las visibles, con skeleton `sdc-shimmer` como testigo; `TarjetaContenido` estrena **blur-up** (placeholder shimmer + fade-in con blur al llegar la imagen); el héroe pide su imagen con `fetchPriority="high"`.
- **i18n ×4**: 13 claves nuevas en es/en/de/fr («Ver todo», «Porque te gusta», «Ajustes», «Reproducción automática», «Carga infinita», «Velocidad/Volumen por defecto», las 3 categorías libres y «Todas las series», y el aviso de autoguardado), con test de paridad.

### Regresión
- `tests/v1380.test.ts`: CATEGORIAS_EXPLORAR (10 categorías, 3 motores, validador con caída honesta), AjustesCine (validador estricto + fusión tolerante), GustosCine (pureza, techo 40 por género, poda a 12, «últimos» a 24 sin duplicados, basura ignorada), recomendadosPara (afinidad, exclusión de «últimos», determinismo, tope), i18n ×4 de las 13 claves, integración (ruta, motor con caché, cron, Cine.tsx con vista/centinela/lazy/Suspense, Reproductor con preload progresivo y ajustes, FilaCarrusel con IO y «Ver todo», TarjetaContenido blur-up, héroe con fetchPriority) y tríada 1.38.0 con README y service worker al día.
...
### Añadido
- **Fusión elegible con el Arena** 🔗 (`arena-enlace.ts` + `EnlaceArena.tsx`): nuevo módulo puro y testeable (`ModoArena`, `esModoArena`, `modoArenaValido`, `enlazadoAlArena`, `enlaceArena`, `alternarModoArena`) que gobierna TODOS los enlaces cruzados: en modo **fusionado** (el de nacimiento) aparece la insignia «Ir al Arena» con degradado violeta-fucsia y el conmutador encendido; en modo **independiente** los enlaces al Arena desaparecen de la UI y StreamDog se anuncia autónoma — perfecto cuando la PWA vive en su propio dominio según `docs/DESPLIEGUE-VERCEL.md`. La preferencia se persiste en `localStorage` con clave versionada (`streamdog.arena.v1.modo`), validación estricta y caída segura al modo base: la basura en el storage nunca rompe nada.
- **«Pacto abierto»** 🤝 (`AvisoLegal.tsx`): el disclaimer que pide la casa, completo y detallado, con 7 bloques canónicos en 4 idiomas: (1) **Qué servimos y qué no** — solo dominio público y metadatos abiertos de Archive, Commons y TVMaze; nada de torrents, cracks ni enlaces piratas; (2) **Nada personal: el problema son los precios** — admiramos a las plataformas, lo que se atraganta son las suscripciones desorbitadas que ya suman más que la factura de la luz; (3) **Colaboración, no enemistad** — un acuerdo mayor: catálogos asequibles, ventanas de prueba, bundles con dominio público, licencias honestas; la puerta abierta a hablar; (4) **Marcas y afiliación** — uso nominativo, sin afiliación ni aval, enlaces siempre al origen oficial; (5) **Privacidad de verdad** — sin cuentas, sin venta de datos, sin rastreadores; todo en tu dispositivo y el chat E2E solo transporta cifrado; (6) **Seguridad auditable** — código abierto, sin malware ni mineros, sandbox estándar; (7) **Legibilidad y retirada** — fuentes a la vista, errores a la cara y retirada inmediata para titulares de derechos. Un pacto duradero: vive en cada versión y evoluciona con ella.

### Mejorado
- **Cabecera de Cine&Series**: nueva fila premium con el conmutador Fusionado/Independiente (segmentos con `aria-pressed` y tooltips explicativos) y el botón «Pacto abierto» (HeartHandshake, degradado esmeralda, animación `sdc-elevarse`) — conexión y legalidad a un toque, con `flex-wrap` para móviles.
- **i18n del módulo**: +24 claves nuevas en es/en/de/fr (conexión Arena, modos y los 7 bloques completos del pacto), con test de completitud que verifica la presencia de las 6 plataformas en los 4 idiomas.
- **Punteros de versión desplazados**: los 20 archivos de test que asertaban `APP_VERSION` apuntan ahora a la 1.35.0; `sw.js` re-versionado (`streamdog-v1.35.0`) para invalidar la caché anterior.

### Regresión
- `tests/v1350.test.ts`: invariantes del módulo de fusión (validación estricta, puerta única `enlaceArena` que respeta el modo independiente, alternancia pura), clave de storage versionada, i18n del pacto completo en 4 idiomas (títulos, cuerpos, plataformas y frases clave: «acuerdo mayor», «enemistad permanente», «suscripciones desorbitadas», «nada en contra»), integración real en `Cine.tsx` y tríada 1.35.0 con README y service worker al día.
...
### Añadido
- **Contenido infinito ♾️** (nuevas `COLECCIONES_ARCHIVE` y `EXITOSOS_MUNDIALES` en `cine.ts`): cinco colecciones curadas de Archive.org — `film_noir`, `sci-fi_horror`, `classic_cartoons`, `classic_tv` (marcadas como series) y `documentaryfilms` — se suman a `feature_films`; cada fila es una consulta real con miles de títulos de dominio público. La fila **«Los títulos más famosos»** consulta 30 clásicos universales por título exacto (Nosferatu, El gabinete del Dr. Caligari, Metropolis, Night of the Living Dead, Charade, Carnaval de almas…) y los ordena por descargas reales de la fuente — popularidad honesta, sin números inventados. La colección que la comunidad renombre desaparece de la fila sin romper nada.
- **Cron empresarial cada hora** ⏰ (nuevos `cine-cron.ts`, `/api/streamdog/cron/actualizar` y `/estado` + `cine-catalogo.ts`): la agregación del catálogo sale de la ruta para que el cron **cocine por adelantado exactamente las mismas cachés** que pega un usuario — portada con sus 10 filas, películas pág. 1-2 y series pág. 1 — y devuelva un **informe JSON** (duración, items calentados, degradación, errores, fuentes) con **historial rotatorio de 24 ejecuciones**. Autorización con `CRON_SECRET` en tiempo constante (Bearer o `x-cron-clave`), sin secreto solo en desarrollo, con aviso honesto. El estado alimenta el **chip ♾️ de la cabecera** con punto de salud en vivo.
- **Despliegue listo para Vercel + dominio propio** 🚀: `vercel.json` declara el cron `0 * * * *` (hora en punto UTC) y su `maxDuration` propio; nuevo **workflow de respaldo** `.github/workflows/cron-catalogo.yml` que refresca cada hora **gratis con GitHub Actions** (para planes Hobby con cron limitado), con informe en el resumen del workflow; y la guía completa `docs/DESPLIEGUE-VERCEL.md` (dashboard/CLI, env vars, dominio propio paso a paso con DNS, checklist de verificación y seguridad).
- **UI premium/VIP estilo Max** ✨ (nuevas `HeroeDestacado.tsx`, `FilaDeportes.tsx`, `FilaProximamente.tsx`, `DialogoProximamente.tsx` + `Cine.tsx`): **héroe destacado** con el nº1 REAL de los famosos (banner grande, degradado, metadatos y Reproducir/Mi lista), **buscador premium** con botón verde BUSCAR (búsqueda inmediata, sin debounce), **chips degradados de salto rápido** que te dejan en la fila que elijas, y badge «Catálogo infinito · Actualizado cada hora» con el estado real del cron.
- **Hoja de ruta que motiva** 🗺️: sección **«Deportes en vivo»** con tarjetas VS de enfrentamientos legendarios (Liverpool–Fulham, Real Madrid–Barcelona, Chelsea–Hull, Bayern–Dortmund, PSG–Marsella, Atlético–Sevilla) con escudos de iniciales y **cuenta atrás real al saque** que corre cada segundo; y las tarjetas **«Muy pronto»** de Viajes, Juegos, Apps y Webs con degradado propio. Pulsar cualquiera abre un **diálogo motivador** en los 4 idiomas — «esto se está cocinando» con un texto que ilusiona sin fechas falsas y un botón que te devuelve al catálogo infinito, que YA está abierto.

### Mejorado
- **i18n del módulo**: +27 claves nuevas en es/en/de/fr (filas nuevas, premium, hoja de ruta y textos motivadores), con tests de completitud.
- **Búsqueda**: la vista de películas suma los famosos a la página 1 para que el buscador herede el catálogo estrella.

### Regresión
- `tests/v1330.test.ts`: constructores puros de colecciones y famosos (saneado de comillas, cláusula OR por título), `normalizarArchiveDoc` con tipo por colección, `claveCine`/fachada de caché, `autorizarCron` (Bearer/x-cron-clave/tiempo constante/sin secreto), historial rotatorio, `saludDe` y `proximaEjecucion`, invariantes de `COLECCIONES_ARCHIVE`/`EXITOSOS_MUNDIALES`, diccionarios 4 idiomas completos y tríada 1.33.0 coherente.

## [1.32.0](https://github.com/FazeUrru/TODO-LOGO-AI/compare/v1.31.0...v1.32.0) · 12 sept 2026, 11:20 — *StreamDog Cine&Series: películas y series gratis reales, reproductor con segundo plano y caché que se repara sola*

> 💡 **En una frase:** StreamDog estrena su módulo de **cine y series gratis con contenido REAL**: el backend propio agrega Wikimedia Commons e Internet Archive (largometrajes de dominio público que se reproducen de verdad) y TVMaze (series con fichas, géneros y episodios), todo con reproductor propio que **sigue sonando en segundo plano** (MediaSession + PiP automático), interfaz en **4 idiomas** y un sistema de caché que **se repara solo** — en el storage y en el service worker.

### Añadido
- **Catálogo real de APIs públicas** 🎬 (nueva `src/lib/streamdog/cine.ts` + `cine-servidor.ts` + `/api/streamdog/cine`, `/detalle` y `/reproducir`): tres fuentes abiertas y legales, sin claves ni piratería — Wikimedia Commons (ficheros de vídeo de dominio público con URL directa: Nosferatu, Night of the Living Dead, Charade, Plan 9, Big Buck Bunny…), Internet Archive (colección `feature_films` con MP4 h.264 jugable) y TVMaze (series con peso, valoraciones y episodios). Normalizadores puros y testeados (`normalizarTvmazeShow`, `normalizarCommonsPage`, `normalizarArchiveDoc`, `elegirVideoArchive` que prefiere el derivado 512kb.mp4), dedupe por título+año, rate-limit por IP y **caché TTL en memoria** (inicio 10 min, listas 20, búsqueda 5) con purga automática.
- **Degradación elegante por fuente** 🛟: cada fuente se pide con tope de tiempo (AbortController); la que falla no tira la respuesta — se devuelve el resto con `degradada: true` y los **chips de fuentes en vivo** (ok/degradada/caída con latencia) pintan la verdad en la cabecera.
- **Reproductor a prueba de segundo plano** ▶️ (nueva `src/components/streamdog/cine/Reproductor.tsx`): el bug «el streaming no va en segundo plano» curado de raíz — (1) el progreso se guarda en el evento `timeupdate` del vídeo, sin timers que el navegador congele; (2) `navigator.mediaSession` declara metadatos y acciones (play/pause/seek/stop) para que el sistema mantenga el foco de medios y dé controles en pantalla de bloqueo; (3) preferencia **«Segundo plano»** persistida: al ocultar la pestaña se pide **Picture-in-Picture automático**. Además: barra con búfer, volumen, velocidades 0.75×–2×, PiP, pantalla completa, reanudar donde lo dejaste, atajos de teclado (espacio/K, ←→, ↑↓, F, M, P, Esc), reintento con re-resolución de URL y "Ver en el origen" cuando no hay vídeo.
- **Multilenguaje es/en/de/fr del módulo** 🌍 (nueva `src/lib/streamdog/cine-i18n.ts`): diccionario propio de ~55 claves en los 4 idiomas del pie clásico de StreamDog, selector en la cabecera con persistencia (hereda del navegador en la primera visita), géneros traducidos y nota honesta: el contenido llega en el idioma de su fuente.
- **Caché que se repara sola (de verdad)** 🔧: el storage del módulo (Mi lista, Seguir viendo, idioma, preferencia) se valida al leer con predicados — JSON roto o forma inesperada → se borra, se reconstruye y el **banner de reparación** lo cuenta con registro horario. El Service Worker estrena el mensaje **`COMPROBAR_SALUD`**: audita su precaché, re-añade entradas faltantes y responde un informe `{tipo:"SALUD", reparado, faltaban, entradas}` — el botón «Comprobar salud» lanza la auditoría a mano. Las respuestas de error (404/5xx) ya no se cachean: la caché no se envenena.
- **UI estilo NetMirror** 🐕 (nuevas `Cine.tsx`, `FilaCarrusel.tsx`, `TarjetaContenido.tsx`, `DetalleModal.tsx`, `BannerReparacion.tsx`): pestaña **«Cine y series»** como portada de StreamDog con héroe de filas en carrusel con scroll-snap, búsqueda con debounce en las 3 fuentes, vistas Inicio/Películas/Series/Mi lista, «Cargar más», pósters con valoración e insignia de fuente, barra de progreso en «Seguir viendo», fichas con episodios por temporada, instalación PWA desde el módulo y footer honesto sobre las fuentes.

### Mejorado
- **PWA actualizada**: manifest con nombre y categorías de cine/entretenimiento, orientación libre, y badge de versión de la sección ahora dinámico (`APP_VERSION`, se acabó el hardcode v1.26.0).
- **i18n de la casa**: el diccionario EN suma las claves del módulo (pestaña, subtítulo y footer).

### Regresión
- `tests/v1320.test.ts`: normalizadores de las 3 fuentes con payloads crudos reales, `elegirVideoArchive` con preferencia h.264, `limpiarHtml` con entidades numeradas y hex, dedupe con prioridad de fuente, paginación, storage con autoreparación (JSON roto, forma inesperada, cuota, purga de progreso), diccionarios de los 4 idiomas completos y sanos, `traducirGenero`, URL builders puros, invariantes estáticos (rutas con rate-limit, SW con `COMPROBAR_SALUD` y versión sincronizada) y tríada 1.32.0 coherente.
