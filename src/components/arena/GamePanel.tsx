...
import { AppWindow, Check, Code2, Copy, ExternalLink, Gamepad2, Maximize2, Minimize2, RefreshCw, TriangleAlert } from "lucide-react";
...
export default function GamePanel({
  ...
  completo: boolean;
  streaming?: boolean;
  /** v1.21.0 — «juego» (Modo Juego) o «app» (web/app pedida por el usuario).
   * Bug del Modo Código: una app fullstack ya no se disfraza de juego. */
  tipo?: "juego" | "app";
}) {
  const esJuego = tipo === "juego";
  const IconoMarco = esJuego ? Gamepad2 : AppWindow;
  ...
  return (
    ...
    >
          ...
          {construyendo ? (
            ...
            <>
              <button
                ...
                title={verCodigo ? (esJuego ? "Volver al juego" : "Volver a la app") : "Ver el código fuente"}
                className={cn(
              ...
              >
                ...
                <span className="hidden sm:inline">{verCodigo ? (esJuego ? "Juego" : "App") : "Código"}</span>
              </button>
              <button
                onClick={() => setReloadKey((k) => k + 1)}
                title={esJuego ? "Reiniciar el juego" : "Reiniciar la app"}
      ...
      {!construyendo && !completo && (
        <div className="flex items-start gap-2 border-b border-amber-500/20 bg-amber-500/10 px-3 py-2">
          ...
          <p className="text-[12px] leading-snug text-amber-200/90">
            {esJuego
              ? "El juego llegó a medias por el límite de tiempo de generación. Pide al modelo «continúa el juego desde donde lo dejaste» o reenvía con el Modo Juego."
              : "La app llegó a medias por el límite de tiempo de generación. Pide al modelo «continúa la app desde donde la dejaste» o reenvía con el Modo Código."}
          </p>
        </div>
      )}
      ...
      {construyendo || verCodigo ? (
        ...
        >
          {code || (esJuego ? "Preparando el motor del juego…" : "Preparando tu app…")}
        </pre>
      ) : (
        <iframe
          key={reloadKey}
          title={esJuego ? "Juego creado por la IA" : "App creada por la IA"}
          sandbox={SANDBOX}
          srcDoc={code}
      ...
      {!construyendo && !verCodigo && !enFull && (
        <div className="flex items-center justify-between gap-2 bg-white/[0.04] px-3 py-1.5">
          ...
          <button
            ...
            title={esJuego ? "Copiar el código HTML del juego" : "Copiar el código HTML de la app"}
          >
