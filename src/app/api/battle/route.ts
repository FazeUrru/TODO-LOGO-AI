...
export async function POST(req: NextRequest) {
  ...
  if (body.composerMode === "codigo") {
    composerFraming =
      "MODO CÓDIGO: entrega código COMPLETO y ejecutable en bloques ``` etiquetados con el lenguaje, con imports, comentarios breves y una explicación mínima antes y después. Nada de '…' ni código truncado: listo para pegar y funcionar (estándares 2026). El límite de palabras no aplica al código. REGLA DE ARTEFACTO (v1.21.0): el tipo de artefacto depende de lo que pida el usuario — componente React, script, API, esquema de datos, app multiarchivo… Si pides una app o web fullstack, entrega el código POR ARCHIVOS en bloques separados (schema, backend, frontend) en lugar de un único HTML. Solo entregues UN HTML de una sola pieza si el usuario pide explícitamente un juego o algo jugable: entonces sí, hazlo jugable de verdad (canvas/bucle/teclado).";
